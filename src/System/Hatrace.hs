...
instance SyscallEnterFormatting SyscallEnterDetails_clone where
  syscallEnterToFormatted SyscallEnterDetails_clone{ child_stack, flags, ptid, newtls, ctid } =
    FormattedSyscall "clone" [ formatArg child_stack, formatArg flags, formatPtrArg "int"  ptid
                             , formatArg newtls, formatPtrArg "int" ctid]
...
getSyscallEnterDetails syscall syscallArgs pid = let proc = TracedProcess pid in case syscall of
  ...
  Syscall_clone -> do
    ...
    let cloneBytes = fromIntegral flags .&. complement 0xFF -- 0xFF == CSIGNAL
    let termSignal = fromIntegral flags .&. 0xFF
    pure $ DetailedSyscallEnter_clone $ SyscallEnterDetails_clone
      ...
      , cloneFlags = fromCInt cloneBytes
      }
